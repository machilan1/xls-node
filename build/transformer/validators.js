...
exports.validateAccNum = validateAccNum;
exports.validateID = validateID;
exports.validateAmount = validateAmount;
exports.validateCrDr = validateCrDr;
exports.validateDate = validateDate;
exports.checkDuplicateCode = checkDuplicateCode;
...
function validateAccNum(acctKind, acctNum) {
    ...
    if (acctKind === "" && acctNum === "")
        return true; // Skip empty account
    if (isNaN(Number(acctKind)) || isNaN(Number(acctNum)))
        return false;
    // Pad the account numbers with leading zeros
    acctKind = acctKind.padStart(5, "0");
    acctNum = acctNum.padStart(9, "0");
    const tmp = 7 * Number(acctKind[0]) +
        1 * Number(acctKind[1]) +
        3 * Number(acctKind[2]) +
        1 * Number(acctKind[3]) +
        3 * Number(acctKind[4]) +
...
function validateID(id) {
    if (id === "" || id === "*")
        return true; // Skip empty ID and ignore *
    // 驗證台灣身分證字號
    ...
    if (idRegex.test(id)) {
        ...
        const checkSum = Math.floor(letterValue / 10) + // 字母數值的十位數
...
function checkDuplicateCode(duplicateCode) {
